    ...
    <v-container fluid>
      <v-row>
        <v-col cols="12" class="pb-0 px-0">
          <v-breadcrumbs :items="['Home', 'Your Cart']" style="font-size: 13px; color: #878787">
            <template v-slot:divider>
              <v-icon color="#648484">mdi-chevron-right</v-icon>
            </template>
          </v-breadcrumbs>
        </v-col>
        <v-col cols="12" class="py-0">
          <v-card elevation="0" class="py-0">
            <v-card-text class="pb-0" style="font-size: 17px; font-weight: bold"
              >Your Cart
            </v-card-text>
            <v-card-text
            ...
            >
              ...
              <h3>
                <span style="color: #ffbd0ff5">Total Price : $ </span>
      ...
      >
        ...
        <v-col cols="12" md="8" class="px-4" v-if="cartItems.length">
          <table class="w-100">
            <thead style="background-color: #cbcbcb">
              <tr class="text-center">
                <th style="font-size: 12px; height: 40px; font-weight: bold">PRODUCT</th>
                <th style="font-size: 12px; height: 40px; font-weight: bold">PRICE</th>
                <th style="font-size: 12px; height: 40px; font-weight: bold">QUANTITY</th>
                <th style="font-size: 12px; height: 40px; font-weight: bold">TOTAL</th>
            ...
            <tbody style="background-color: #eeeeee">
              <tr v-for="cartItem in cartItems" :key="cartItem.id">
                <td style="width: 55%">
                  <v-row class="mx-4 my-2 align-center" elevation="0">
                    <v-col cols="4" class="pa-0 ma-0 px-1 text-center position-relative">
                      <img :src="cartItem.thumbnail" alt="" class="w-100 h-100 e" />
                    </v-col>
                    <v-col cols="7">
                      <v-card-title
                        style="
                      ...
                      >
                      <v-card-subtitle>({{ cartItem.brand }})</v-card-subtitle>
                      <v-card-subtitle>Only have {{ cartItem.stock }}In Stock</v-card-subtitle>
                    </v-col>
                ...
                <td style="width: 15%" class="text-center">
                  <v-card-text style="font-size: 14px; font-weight: 700"
                ...
                <td style="width: 15%" class="text-center">
                  <v-card-text style="font-size: 12px" class="w-100 py-2 align-center">
                    $ {{ (cartItem.price * cartItem.quantity).toFixed(2) }}
                  </v-card-text>
                </td>
                <td>
                  <v-icon @click="deleteItem(cartItem.id)">mdi-close</v-icon>
                </td>
              </tr>
            </tbody>
          </table>
          <v-col cols="12">
            <div class="d-flex align-center justify-center">
              ...
              <v-card-text style="color: red; font-weight: bold" v-if="cartItems.length"
                >Secure Shipping grance
              </v-card-text>
            </div>
          </v-col>
        </v-col>
        <v-col cols="12" md="4" class="px-4 mt-8 mt-sm-0" v-if="cartItems.length">
          <v-card elevation="0">
            <v-card-text class="font-weight-bold">ORDER SUMMARY</v-card-text>
            <v-divider length="100%" color="black"></v-divider>
            <v-divider length="100%" color="black"></v-divider>
            <v-divider length="100%" color="black"></v-divider>
            <v-card-text class="py-2 font-weight-bold d-flex justify-space-between text-center"
              ><span style="font-size: 12px">SubTotaL :</span>
              <p style="font-size: 11px; font-weight: bolder; color: #878787">
                ${{ calcTotalFreeShippingPrice }}
              </p></v-card-text
            >
            <v-card-text style="font-size: 12px"> Get Shipping estimate. </v-card-text>
            <select
              name=""
              id=""
              class="w-100 form-select py-1 px-3"
              style="
                border: 1px solid #777c6d;
                color: #334443;
                border-radius: 30px;
                font-size: 14px;
              "
            ...
            <div class="selectors d-flex align-center justify-space-between mt-4">
              <select
                name=""
                id=""
                class="py-1 px-3"
            ...
            <v-card-actions class="d-flex align-center text-center justify-center ma-0 pa-0 my-4">
              <v-btn
                elevation="0"
                style="text-transform: none; border-radius: 20px; background-color: #ffbd0ff5"
                class="w-100"
          ...
          <v-divider length="100%" color="black"></v-divider>
          <v-divider length="100%" color="black"></v-divider>
          <v-checkbox label="i agree with terms & conditions"> </v-checkbox>

          <v-card-actions class="my-0 py-0 d-flex flex-column align-center justify-center">
            <v-btn
              elevation="0"
              style="text-transform: none; background-color: #ffbd0ff5"
              class="w-100"
              density="compact"
              height="40"
              width="80"
            ...
            <v-btn
              elevation="0"
              style="text-transform: none; border: 1px solid #cbcbcb"
              class="w-100 mt-1"
              density="compact"
              height="40"
              width="80"
...
export default {
  ...
  methods: {
    ...mapActions(useCartStore, ["deleteItem","loadCartItem","saveCartItem"]),
    toCheckOut(){
      this.saveCartItem();
  ...
  computed: {
    ...
    calcTotalFreeShippingPrice() {
      ...
      this.cartItems.forEach((product) => {
        total += product.price * product.quantity * (1 - product.discountPercentage / 100);
      });
